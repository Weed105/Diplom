            ...
            <form action="{{ route('admin.enrollments.update', [$enrollment->id]) }}" method="POST"
                ...
                @if (auth()->user()->isInstitution())
                    <input type="hidden" name="user_id"
                        value="{{ isset($enrollment) && $enrollment->user ? $enrollment->user->id : '' }}">
                @else
                    <div class="form-group {{ $errors->has('user_id') ? 'has-error' : '' }}">
                        ...
                        <select name="user_id" id="user" class="form-control select2" required>
                            @foreach ($users as $id => $user)
                                <option value="{{ $id }}"
                                    {{ (isset($enrollment) && $enrollment->user ? $enrollment->user->id : old('user_id')) == $id ? 'selected' : '' }}>
                                    {{ $user }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group {{ $errors->has('course_id') ? 'has-error' : '' }}">
                    ...
                    <select name="course_id" id="course" class="form-control select2" required>
                        @foreach ($courses as $id => $course)
                            <option value="{{ $id }}"
                                {{ (isset($enrollment) && $enrollment->course ? $enrollment->course->id : old('course_id')) == $id ? 'selected' : '' }}>
                                {{ $course }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                    ...
                    @foreach (App\Enrollment::STATUS_RADIO as $key => $label)
                        <div>
                            <input id="status_{{ $key }}" name="status" type="radio"
                                value="{{ $key }}"
                                {{ old('status', $enrollment->status) === (string) $key ? 'checked' : '' }} required>
